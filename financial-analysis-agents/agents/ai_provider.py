...
class GroqWrapper:
    ...
    def __init__(self, provider, json_mode: bool = False):
        ...
        if not GROQ_AVAILABLE:
            raise ImportError("Libreria 'groq' non installata. Esegui: pip install groq")
    ...
    def generate_stream(self, prompt):
        ...
        try:
            ...
            response = self.client.models.generate_content(
                model=self.provider.current_model_name,
                contents=prompt,
                config={'response_mime_type': 'text/plain'} if not self.json_mode else {'response_mime_type': 'application/json'},
            )
...
class AIProvider:
    ...
    def __init__(self, api_key: Optional[str] = None, provider_type: str = "gemini", model_name: Optional[str] = None):
        ...
        if self.provider_type == "gemini":
            ...
            self.log_debug(f"🤖 AI Provider impostato su Groq: {self.current_model_name}")
